...
| 2026-07-19 | Kling v3 Pro | ⚠️ **PASS with a flagged identity-drift issue** (attempt 2) | 15s, 1924×1076 (correct 16:9). Re-composed the two-shot start frame with aspect ratio set correctly first, added explicit menu-continuity language to the motion prompt. Anatomy, menu continuity, hand-off, background stability, and phantom-limb checks all pass. **Camera was not fully static as instructed** — it drifted from a wide two-shot into a tight solo close-up on Maria by ~9s, cropping James out of frame. **2026-07-19, later review at higher zoom: Maria's face shows visible identity drift during that close-up** — eyebrow shape reads thicker/flatter than her locked reference, face structure reads slightly narrower through the lower face. Missed on first QA pass because wide two-shot frames didn't resolve enough facial detail to catch it; caught on closer inspection of the solo close-up stills. Working theory: the uncommanded camera push-in is likely the cause — the larger the reframe away from the start image's original framing, the more room Kling has to improvise facial detail not tied to the reference. |

**VERDICT: CONDITIONAL PASS.** The core anatomy/object-interaction risk (the reason this gate exists) is cleared — no detached limbs, no menu morphing, stable background. However, a real face-identity-drift risk was found during an uncommanded camera move, which directly threatens the character-lock strategy the whole project depends on. Before fully committing Kling v3 Pro as the workhorse for all 5 episodes, run a follow-up test specifically targeting this: generate another short clip that deliberately includes a push-in/close-up on one character, and check identity consistency at the tightest framing. If drift is confirmed as camera-move-correlated, mitigations to try: stronger identity-lock language in video motion prompts (not just still-composition prompts), and/or scene-director guidance to avoid large uncommanded reframes — possibly by keeping shots shorter or re-anchoring each new framing with a fresh reference-locked start image rather than letting one continuous generation drift through multiple framings.
